...
const EditProject = () => {
    ...
    const [status, setStatus] = useState('System_Initializing');
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (image) {
                setStatus('Compressing_New_Image...');
                bodyFormData.append('image', image);
            }

            setStatus('Transmitting_Buffer...');
            const response = await fetch(`${backendUrl}/api/project/update/${id}`, {
                method: 'POST',
                headers: {
        ...
        <div className="space-y-12 pb-20">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
                <div>
                    ...
                    <h1 className="text-5xl lg:text-7xl font-black uppercase tracking-tighter italic">Edit Project</h1>
                </div>
            ...
            <div className={`relative w-full max-w-5xl bg-[#0c0c0c] border border-white/10 rounded-[2rem] shadow-[0_0_100px_rgba(37,99,235,0.1)] flex flex-col md:flex-row overflow-hidden min-h-[600px] transition-all duration-500 ${loading ? 'opacity-50 pointer-events-none' : ''}`}>
                {/* LEFT PANEL */}
                    ...
                    {loading && (
                        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/5 backdrop-blur-[2px]">
                            <div className="flex flex-col items-center gap-4">
                                ...
                                <p className="font-mono text-xs uppercase tracking-[0.5em] text-blue-500 animate-pulse">Sync_In_Progress...</p>
                            </div>
                        </div>
                    )}
                    ...
                    <form onSubmit={handleSubmit} className="space-y-10">
                        ...
                        <div className="relative group overflow-hidden p-8 border border-white/5 bg-gradient-to-br from-white/[0.02] to-transparent rounded-3xl flex flex-col items-center justify-center gap-4 hover:border-blue-600/30 transition-all cursor-pointer">
                            ...
                            <label htmlFor="image" className="cursor-pointer flex flex-col items-center gap-4">
                                <div className="w-16 h-16 rounded-full border border-blue-600/20 flex items-center justify-center text-blue-600 group-hover:bg-blue-600 group-hover:text-white transition-all shadow-2xl overflow-hidden">
                                    {image ? (
                                        <img className="w-full h-full object-cover" src={URL.createObjectURL(image)} alt="" />
                                    ) : currentImage ? (
                                        <img className="w-full h-full object-cover" src={currentImage} alt="" />
                                    ) : (
                                        <Upload size={24} />
                                    )}
                                </div>
                                <p className="relative z-10 text-[9px] font-mono uppercase text-neutral-500 tracking-[0.4em]">
                                    {image ? image.name : currentImage ? "Change_Project_Image" : "Select_Project_Image"}
                                </p>
                            </label>
                        </div>
